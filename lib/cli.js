...
function upperSort(a, b) {
  //ex: a=factory-disclaimer b=factory => a < b
  if (a.key.startsWith(b.key) && a.key.charAt(b.key.length) == '-' && Object.prototype.toString.call(b.value) == '[object Object]')
    return -1

  //ex: a=factory b=factory-disclaimer => a > b
  else if (b.key.startsWith(a.key) && b.key.charAt(a.key.length) == '-' && Object.prototype.toString.call(a.value) == '[object Object]')
    return 1;
...
function sortObject(o) {
  ...
  sorter.sort(upperSort);

  for (var index = 0; index < sorter.length; index++) {
    const { key, value } = sorter[index];
